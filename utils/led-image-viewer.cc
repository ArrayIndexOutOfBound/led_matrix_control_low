...
//Text setup
  ...
  const char *bdf_font_file = "./spleen-16x32.bdf";
  int x_orig = 32;
  int y_orig = 0;
...
//      DisplayAnimation(file_imgs[i], matrix, offscreen_canvas);
		FileInfo *file = file_imgs[i];
		tmillis_t duration_ms = (file->is_multi_frame
                                 ? file->params.anim_duration_ms
                                 : file->params.wait_ms);
		rgb_matrix::StreamReader reader(file->content_stream);
		int loops = file->params.loops;
		tmillis_t end_time_ms = GetTimeInMillis() + duration_ms;
		tmillis_t override_anim_delay = file->params.anim_delay_ms;
		uint32_t delay_us = 0;
		if (outline_font) {
		...
		rgb_matrix::DrawText(offscreen_canvas, font, x_orig, y_orig + font.baseline(),
