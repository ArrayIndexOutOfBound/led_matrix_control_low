...
int main(int argc, char *argv[]) {
  ...
  rgb_matrix::Color outline_color(0,255,0);
  rgb_matrix::Color Vert_color(0,255,0);
  rgb_matrix::Color Rouge_color(255,0,0);
...
//    Image img_PMR("./images/PMR_32x32.png");
	Image img_PMR("./images/Parking-32x32.png");
	...
	//Flèche	
		for (int x = 0; x < nx_FG; x++) {
		  for (int y = 0; y < ny_FG; y++) {
			...
			if (c.alphaQuantum() < 256) {
				offscreen->SetPixel(x+96, y, ScaleQuantumToChar(c.redQuantum()),
								   ScaleQuantumToChar(c.greenQuantum()),
								   ScaleQuantumToChar(c.blueQuantum()));
			}
		  }
		}
		x_orig = 64 - 16*strlen(text_buffer)/2;
...
*/	
    ...
    if (strcmp(text_buffer,"0") && strcmp(text_buffer,"-1")) {
	  // Texte	
	  rgb_matrix::DrawText(offscreen, font, 16, 33 + font.baseline(),
                           Vert_color, NULL, "OUVERT",
                           letter_spacing);
	} else {
	  rgb_matrix::DrawText(offscreen, font, 8, 33 + font.baseline(),
                           Rouge_color, NULL, "COMPLET",
                           letter_spacing);
    }		
